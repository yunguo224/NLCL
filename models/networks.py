...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'mlp_patch_flatten':
        net = PatchSampleFlattern(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'mlp_patch_square':
        net = PatchSampleSquare(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc, patch_w=opt.sample_patch_size)
    elif netF == 'strided_conv':
        net = StridedConvF(init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    elif netF == 'non_localOne':
        net = PatchSampleNonlocalOneGroup(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    else:
...
# we sample the real patch rather than the pixel
class PatchSampleFlattern(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleFlattern, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) #(B, H*W, C)
            sample = torch.zeros((B, num_patches, self.patch_size, C), device=feats[0].device)
            if num_patches > 0:
                if patch_ids is not None:
                    patch_id = patch_ids[feat_id]
                else:
                    patch_id = torch.randperm(feat_reshape.shape[1]-self.patch_size, device=feats[0].device)
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
                # patch_id shape: 256
                # x_sample shape: B, num, patchsize, C
                for num, id in enumerate(patch_id):
                    sample[:,num,:,:] = feat_reshape[:, id:id+self.patch_size, :] # reshape(-1, x.shape[1])
                # final x_sample: B*num, patchsize*C
                x_sample = sample.flatten(2,3)
...
class Generator(nn.Module):
  def __init__(self, inchannel, outchannel):
    ...
    ksize = 3
    convs = []
    convs.append(nn.Conv2d(self.inchannel, G0, ksize, padding=(ksize - 1) // 2, stride=1))
    for i in range(16):
        convs.append(ResBlock(G0))
    convs.append(conv_ycf(G0, G0))
    convs.append(nn.Conv2d(G0, self.outchannel, ksize, padding=(ksize - 1) // 2, stride=1))
    self.gen = nn.Sequential(*convs)
  def forward(self,input_b, layers=[], encode_only=False):
      if len(layers) > 0:
          feat = input_b
          feats = []
...
class PatchSampleSquare(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[], patch_w=4):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            sample = torch.zeros((B, num_patches, self.patch_w * self.patch_w * C), device=feats[0].device)
            if num_patches > 0:
                if patch_ids is not None:
                    patch_id = patch_ids[feat_id]
                else:
                    ...
                    for i in range(0, num_patches):
                        patch_id[i,0] = random.randint(0, feat_reshape.shape[1]-self.patch_w)
                        patch_id[i,1] = random.randint(0, feat_reshape.shape[2]-self.patch_w)
                    # patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
                # patch_id shape: 256
                # x_sample shape: B, num, patchsize, C
                for num, id in enumerate(patch_id):
                    patch = feat_reshape[:,int(id[0]):int(id[0])+self.patch_w, int(id[1]):int(id[1])+self.patch_w, :]
...
def getTensorLocs(tensor, center_patch_loc, patch_size, search_size, stride,device):
    ...
    starting_loc_h = half if (center_patch_loc[0]-search_half-half < 0) else center_patch_loc[0]-search_half
    starting_loc_w = half if (center_patch_loc[1]-search_half-half < 0) else center_patch_loc[1]-search_half
    ending_loc_h = (height-half) if (center_patch_loc[0]+search_half+half > height) else center_patch_loc[0]+search_half
    ending_loc_w = (width-half) if (center_patch_loc[1]+search_half+half > width) else center_patch_loc[1]+search_half
    patches = torch.zeros((tensor.shape[0],tensor.shape[1],patch_size, patch_size), device=device)
...
def debug_seeNonlocal(Tensor, locs, half):
    import cv2
    from util.util import tensor2im
    for loc in locs:
        seeTensor = getTensorPatch(Tensor, loc, half)
        numPatch = tensor2im(seeTensor)
        numPatch = cv2.resize(numPatch,(256,256))
        cv2.imshow('non local patch',numPatch)
        cv2.waitKey()
...
class PatchSampleNonlocalOneGroup(nn.Module):
    ...
    def forward(self, feats, num_patches=256, patch_ids=None):
        ...
        if patch_ids is None:  # calculate the num_patches non-local keys in raw image
            ...
            sample_loc = [random.randint(self.patch_size, H - self.patch_size),
                          random.randint(self.patch_size, W - self.patch_size)]
            sample_patch = getTensorPatch(img, sample_loc, self.half)
            locs, patches = getTensorLocs(img, sample_loc, self.patch_size, self.search_size, self.stride, img.device)
